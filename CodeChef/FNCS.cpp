...
lint fsum[RT];
int a[N],fl[N],fr[N],ctr[RT][N],n,rt=0;
struct PrefixSum{
	lint pre[N],blk[RT];
	PrefixSum(){
...
inline void add_ctr(int a[],int fl[],int fr[],int rt,int n){
	for(int i=0;i<rt;i++){
		a[fl[i]]++,a[fr[i]+1]--;
	}
	for(int i=1;i<n;i++){
...
int main(){
	...
	for(int i=0;i<=(n-1)/rt;i++){
		add_ctr(ctr[i],fl+i*rt,fr+i*rt,min(rt,n-i*rt),n);
	}
	for(int i=0;i<n;i++){
