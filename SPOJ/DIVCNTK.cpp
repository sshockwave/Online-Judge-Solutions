...
int bs;
inline ull Main(){
	ull n=next_num<ull>(),k=next_num<ull>();
	for(rt=0;(ull)rt*rt<n;rt++);
	bs=0;
	for(lint l=1,r;(ull)l<=n;l=r+1){
	...
	for(int p1=bs,p2=bs;--i>=0;){//F
		...
		for(;p2>=1&&rend[p2]>=sq;p2--){
			f[p2]+=max(tag[p2]-i,0)*(k+1);
			tag[p2]=-1;
		}
		for(int j=bs;rend[j]>=sq;j--){
			ull ans=0;
			lint nj=rend[j];
			for(ull c=1;nj>0;c+=k,nj/=pri[i]){
				int id=nj<=rt?nj:bs-(n/nj)+1;
				ans+=(f[id]+max(tag[id]-i,0)*(k+1))*c;
			}
			f[j]=ans;
		}
	}
	return f[bs]+max(tag[bs]-i,0)*(k+1);
}
int main(){
#ifndef ONLINE_JUDGE
